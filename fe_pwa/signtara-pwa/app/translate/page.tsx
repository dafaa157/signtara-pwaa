...
export default function TranslatePage() {
  ...
  const recentFeaturesRef = useRef<number[][]>([]); // Untuk masuk ke AI (Normalisasi)
  const recentRawHandsRef = useRef<number[][]>([]); // BARU: Khusus untuk deteksi gerakan murni
  ...
  const handleHolisticResults = async (results: any) => {
    ...
    // 1. AMBIL TITIK HIDUNG SEBAGAI JANGKAR (NORMALISASI)
    ...
    let feat: number[] = []; // Array Normalisasi (Untuk dikirim ke AI)
    let rawHands: number[] = []; // Array Murni (Hanya untuk cek kecepatan tangan)

    // A. Proses Normalisasi (Sesuai Syarat AI)
    ...
    // B. Proses Ekstraksi Tangan (Normalisasi untuk AI, dan Raw untuk Kecepatan)
    if (results.leftHandLandmarks) {
      results.leftHandLandmarks.forEach((lm: any) => {
        feat.push(lm.x - anchorX, lm.y - anchorY, lm.z - anchorZ); // Normalisasi
        rawHands.push(lm.x, lm.y, lm.z); // Raw Murni
      });
    } else { 
    ...
    // 2. LOGIKA STATE MACHINE MENGGUNAKAN DATA RAW (KEBAL TREMOR KEPALA)
    if (recentRawHandsRef.current.length === 5 && !isPredictingRef.current) {
      let totalVelocity = 0;
      
      for (let i = 1; i < recentRawHandsRef.current.length; i++) {
        let frameVelocity = 0;
        // Hanya cek pergerakan pada 126 titik tangan murni!
        for (let j = 0; j < 126; j++) {
      ...
      if (meanVelocity > MOTION_THRESHOLD) {
        ...
        if (!isSigningRef.current) {
          ...
          recordedSequenceRef.current = [...recentFeaturesRef.current]; // Yang direkam tetap data normalisasi
        } else {
  ...
  const startCamera = async () => {
    try {
      ...
      recentRawHandsRef.current = []; // Reset juga buffer raw-nya
      recordedSequenceRef.current = [];
      isSigningRef.current = false;
